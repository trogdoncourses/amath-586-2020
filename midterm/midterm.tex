...
This entire exam concerns system of ODEs called the \emph{Toda lattice}.  The system is defined using momenta $p_j$, $j = 0, \pm 1, \pm 2,\ldots$ and positions $q_j$, $j = 0, \pm 1, \pm 2, \ldots$:
\begin{align}\label{eq:toda}
  \begin{split}
  q_j'(t) &= p_j(t),\\
  p_j'(t) &= e^{-(q_{j}(t) - q_{j-1}(t))} - e^{-(q_{j+1}(t) - q_j(t))}, \quad j = 0, \pm 1, \pm 2,\ldots.
  \end{split}
\end{align}
...
\begin{enumerate}[label={\bf Problem~{\arabic*}:}]
  ...
  \begin{align} \label{eq:toda_ab}
    ...
    b_j'(t) &= 2 (a_{j}^2(t) - a_{j-1}^2(t)),  \quad j = 0, \pm 1, \pm 2,\ldots.
    \end{split}
  \end{align}
...
\item One finite-dimensional approximation of \eqref{eq:toda_T} is to just take a finite section (a square subblock on the diagonal) of both $T$, $S$:
  \begin{align*}
    T_N(t) &= \begin{bmatrix}
      b_1(t) & a_1(t)\\
      a_1(t) & b_{2}(t) & a_{2}(t)\\
